...
/**
 * A mock of the completion certificate, tilted slightly out of the page so it
 * reads as an object rather than a screenshot. Decorative — every line printed
 * on it is repeated as real text in the panel beside it.
 */
function CertificatePlate({ view }: { view: AiCourseView }) {
  ...
  return (
    <motion.div
      aria-hidden="true"
      initial={reduce ? false : { opacity: 0, y: 30, rotateY: -12 }}
    ...
    >
      ...
      <div className="absolute -inset-6 -z-10 rounded-3xl bg-brand-500/25 blur-[70px]" />
    </motion.div>
  );
}
