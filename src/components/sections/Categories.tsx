...
/** Keyed by `icon` on each category item. */
const CATEGORY_ICON = {
  laptop: Laptop,
  brain: BrainCircuit,
  pen: PenTool,
    ...
    >
      ...
      <div className="container-page">
        ...
        <ul className="mt-12 grid gap-6 sm:grid-cols-2">
          {visible.map((item, i) => {
            const Icon = CATEGORY_ICON[item.icon];
            return (
              <Reveal as="li" key={item.title} delay={i * 60}>
                ...
                >
                  ...
                  {/* Illustration slot: an oversized glyph with a soft bloom,
                      standing in for artwork until real renders land. */}
                  <div
                    aria-hidden="true"
                    className="pointer-events-none absolute -right-4 bottom-2 transition-transform duration-500 group-hover:scale-105 lg:right-2"
                  >
                    <div className="relative grid size-40 place-items-center lg:size-48">
                      <span className="absolute size-28 rounded-full bg-brand-400/25 blur-3xl lg:size-32" />
                      <Icon
                        className="relative size-24 text-white/25 transition-colors duration-500 group-hover:text-white/40 lg:size-28"
                        strokeWidth={1.1}
                      />
                    </div>
                  </div>
                  ...
                  <div className="relative flex items-center gap-3">
                    <span className="grid size-12 place-items-center rounded-full bg-white/10 text-white ring-1 ring-white/20 ring-inset transition-all duration-300 group-hover:bg-white group-hover:text-ink">
